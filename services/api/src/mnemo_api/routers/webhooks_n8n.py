...
in the `Idempotency-Key` header) to make replays safe.
"""
...
async def n8n_webhook(
    ...
    async for session in get_session():
        # Idempotency: short-circuit if we've already processed this key.
        ...
        if event == "note-ready":
            ...
            response = await _handle_note_ready(session, ready)
...
async def _handle_note_ready(session: Any, payload: N8NNoteReadyIn) -> dict[str, Any]:
...
async def _handle_note_failed(session: Any, payload: N8NNoteFailedIn) -> dict[str, Any]:
    note = await session.get(Note, payload.note_id)
    if note is None:
        return {"status": "not_found"}
    note.status = NoteStatus.failed.value
    note.error_message = payload.error_message[:2_000]
    return {"status": "accepted"}


# Re-export the select symbol so mypy keeps the import for a future query.
_ = select
